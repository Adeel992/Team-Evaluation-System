...
function innovation_setup() {
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'innovation_custom_background_args',
			array(
...
function innovation_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'innovation' ),
			'id'            => 'sidebar-1',
...
function innovation_scripts() {
	wp_enqueue_style( 'innovation-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'innovation-style', 'rtl', 'replace' );

	wp_enqueue_script( 'innovation-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
